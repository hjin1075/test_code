...
class TestDebugApi:
    ...
    @pytest.mark.parametrize('base_info,testcase', get_testcase_yaml("./testcase/debugApi/updateUser.yaml"))
    def test_update_user(self, base_info,testcase):
        RequestBase().specification_yaml(base_info,testcase)

    @allure.story(next(c_id) +"删除用户")
    @pytest.mark.run(order=3)
    @pytest.mark.parametrize('base_info,testcase', get_testcase_yaml("./testcase/debugApi/deleteUser.yaml"))
    def test_delete_user(self, base_info,testcase):
        RequestBase().specification_yaml(base_info,testcase)

    @allure.story(next(c_id) +"查询用户")
    @pytest.mark.run(order=3)
    @pytest.mark.parametrize('base_info,testcase', get_testcase_yaml("./testcase/debugApi/queryUser.yaml"))
